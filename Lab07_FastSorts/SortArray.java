...
public class SortArray
{
    ...
    int partition(T[] a, int first, int last)
    {
        ...
        if (last - first + 1 >= 8 && last - first + 1 <= 40)
        {
    ...
    public static <T extends Comparable<? super T>>
    void findCandidatePivotIndexes(ArrayList<Integer> list, T[] a, int first, int last)
    {
        int spaceCheck = a.length/9;
        int numberOfIndexes = 9;
        int mid = first + (last - first)/2;

        if (list.size() < numberOfIndexes)
        {
            if ((last - first) > (a.length/8))
            {
                list.add(mid);
                findCandidatePivotIndexes(list, a, first, mid);
                findCandidatePivotIndexes(list, a, mid, last);
            }
        }
...
//        while (!done)
//        {
//            if ((tempFirst + (tempMid - tempFirst))/2 > spaceCheck)
//            {
//                tempMid = (tempFirst + (tempMid - tempFirst))/2;
//                list.add(tempMid);
//            }
//            if ((tempMid + (tempLast - tempMid))/2 > spaceCheck)
//        }
    }
...
//                }
     ...
     * @param maxNumber the largest possible value in the array
     */
    public static void binaryRadixSort(Integer[] a, int maxNumber)
    {
        ...
        // CONVERT all numbers in array to 2 base
        // FIND how many numbers are in the largest
        // ADD zeros in front of numbers that are not the same length as max number
        // WHILE we have digits in our numbers
        //     ADD each number to the appropriate bucket based on last digit
        //     COPY over numbers from bucket to a temp arraya
        //     EMPTY bucket
        //     MOVE up a digit
        // MOVE temp array to our original array
        // PRTIN our sorted array

        Integer[] temp = a;
        int bitMask = 1;
        int result;
        for (int r = 0; r < Integer.SIZE - Integer.numberOfLeadingZeros(maxNumber); r++)
        {
            indexBucket0 = 0;
            indexBucket1 = 0;
            ...
            for (int i = 0; i < a.length; i++)
            {
                result = temp[i] & bitMask;
                if (result == 0)
                {
                    buckets[0][indexBucket0] = temp[i];
                    indexBucket0++;
                }
                else
                {
                    buckets[1][indexBucket1] = temp[i];
                    indexBucket1++;
                }
            }
