...
public class MaxSumInRectangularGrid
{
    ...
    private String maxPath(int r, int c)
    {
        if (r <= 0)
        {
            return "[" + r + ", " + c + "]:" + this.grid[r][c];
        }
        else
        {
            if (this.path[r][c] == FROM_ABOVE_LEFT)
            {
                return maxPath(r - 1, c - 1) + " ==> [" + r + ", " + c + "]:" + this.grid[r][c];
            }

            if (this.path[r][c] == FROM_ABOVE_CENTER)
            {
                return maxPath(r - 1, c) + " ==> [" + r + ", " + c + "]:" + this.grid[r][c];
            }

            if (this.path[r][c] == FROM_ABOVE_RIGHT)
            {
                return maxPath(r - 1, c + 1) + " ==> [" + r + ", " + c + "]:" + this.grid[r][c];
            }
        }
