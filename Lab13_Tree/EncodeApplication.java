...
public class EncodeApplication
{
    ...
    public HuffmanTree[] createInitialTrees(int[] count)
    {
        ...
        for (int i = 0; i < count.length; i++)
        {
            if (count[i] > 0)
            {
                ...
                HuffmanCode code = new HuffmanCode(characters, count[i]);
    ...
    public void createHuffmanTree()
    {
        while (this.numberOfTrees > 1)
        {
            ...
            int smallest1 = findSmallest(this.numberOfTrees);
            swap(smallest1, this.numberOfTrees - 1);

            System.out.println("--->Smallest tree moved to the position " + (this.numberOfTrees - 1));

            // swap second smallest to the end

            int smallest2 = findSmallest(this.numberOfTrees - 1);
            swap(smallest2, this.numberOfTrees - 2);
            ...
            List<Character> smallest1HC = this.myTrees[this.numberOfTrees - 1].getRootData().getSymbols();
            List<Character> smallest2HC = this.myTrees[this.numberOfTrees - 2].getRootData().getSymbols();
            List<Character> symbol = new ArrayList();
            symbol.addAll(smallest1HC);
            symbol.addAll(smallest2HC);

            int combinedCount = this.myTrees[this.numberOfTrees - 1].getRootData().getFrequency() + this.myTrees[this.numberOfTrees - 2].getRootData().getFrequency();

            HuffmanCode code = new HuffmanCode(symbol, combinedCount);
            HuffmanTree tree = new HuffmanTree(code, this.myTrees[this.numberOfTrees - 1], this.myTrees[this.numberOfTrees - 2]);

            this.myTrees[this.numberOfTrees - 2] = tree;
            this.myTrees[this.numberOfTrees - 1] = null;

//            System.out.println("--->Combined tree created: " + symbol + " -> " + combinedCount);
            System.out.println("--->Combined tree created: " + tree.getCurrentData().toString());
